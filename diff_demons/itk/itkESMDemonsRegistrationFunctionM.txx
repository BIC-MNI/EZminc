/*=========================================================================
  ...
  Module:    $RCSfile: itkESMDemonsRegistrationFunctionM.txx,v $
  Language:  C++
...
#ifndef __itkESMDemonsRegistrationFunctionM_txx
#define __itkESMDemonsRegistrationFunctionM_txx
...
namespace itk
  ...
  ESMDemonsRegistrationFunctionM<TFixedImage,TMovingImage,TDeformationField,TMask>
  ::ESMDemonsRegistrationFunctionM()
  {
    ...
    m_MovingImageMaskWarper = MaskWarperType::New();
    m_MovingImageMaskWarper->SetInterpolator ( m_MovingImageMaskInterpolator );
    m_MovingImageMaskWarper->SetEdgePaddingValue ( NumericTraits<MaskPixelType>::ZeroValue() );
   ...
   */
  template <class TFixedImage, class TMovingImage, class TDeformationField,class TMask>
  void ESMDemonsRegistrationFunctionM<TFixedImage,TMovingImage,TDeformationField,TMask>
  ::InitializeIteration()
  {
    ...
    m_MovingImageWarper->SetDisplacementField ( this->GetDeformationField() );
#else    
    ...
    if ( this->GetMovingImageMask() ) //we are using masks
      {
        // Compute warped moving image
        m_MovingImageMaskWarper->SetOutputOrigin ( this->m_FixedImageOrigin );
        m_MovingImageMaskWarper->SetOutputSpacing ( this->m_FixedImageSpacing );
        m_MovingImageMaskWarper->SetOutputDirection ( this->m_FixedImageDirection );
        m_MovingImageMaskWarper->SetInput ( this->GetMovingImageMask() );
        
#if ( ITK_VERSION_MAJOR > 3 ) 
        m_MovingImageMaskWarper->SetDisplacementField ( this->GetDeformationField() );
#else    
        m_MovingImageMaskWarper->SetDeformationField ( this->GetDeformationField() );
#endif
        
        m_MovingImageMaskWarper->GetOutput()->SetRequestedRegion ( this->GetDeformationField()->GetRequestedRegion() );
        m_MovingImageMaskWarper->Update();
   ...
   */
  template <class TFixedImage, class TMovingImage, class TDeformationField,class TMask>
  typename ESMDemonsRegistrationFunctionM<TFixedImage,TMovingImage,TDeformationField,TMask>
  ::PixelType
  ESMDemonsRegistrationFunctionM<TFixedImage,TMovingImage,TDeformationField,TMask>
                    ...
                    const FloatOffsetType& itkNotUsed ( offset ) )
  {
    ...
    if ( this->GetMovingImageMask() )
      {
        movingMask = m_MovingImageMaskWarper->GetOutput()->GetPixel ( index );
      }
    ...
    //TODO: for now mask is used in  binary way (on/off)
    //TODO: update it to use it as a weight (?)
